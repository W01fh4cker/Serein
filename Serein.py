...
def fofa():
    try:
        ...
        if res["error"] == "true":
            showerror('出错了!', '请检查您的fofa语句、base64后的fofa语句是否正常；\n若确实无异常，请立即联系微信W01fh4cker！报错内容：'+ str(error))
            text3.insert(END,chars="[×]出错了!请检查您的fofa语句、base64后的fofa语句是否正常；若确实无异常，请立即联系微信W01fh4cker！，报错内容："+ str(error)+"\n")
            text3.see(END)
...
button4 = ttk.Button(group3,text="向日葵RCE一把梭",command=xrk_rce_gui,width=20,bootstyle="info")
