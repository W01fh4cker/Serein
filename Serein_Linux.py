...
def hunter_info():
    ...
    hunter_button = ttk.Button(hunter_info, text="点击保存（若需修改配置，请自行修改当前目录下的【hunter配置.conf】）", command=hunter_saveit_twice,
                               width=61, bootstyle="info")
...
def hunter_query():
    ...
    url = 'https://hunter.qianxin.com/openApi/search?api-key=' + str(api_key) + '&search=' + str(
        query_sentence) + '&page=' + str(hunter_pagenum_to_query) + '&page_size=' + str(
        hunter_per_page_size) + '&is_web=' + str(
        hunter_asset_type) + '&start_time=' + str(hunter_start_time) + '&end_time' + str(
        hunter_end_time) + '&status_code=' + str(hunter_status_code)
    headers = {
...
button7 = ttk.Button(group3, text="SonicWall SSL-VPN RCE一把梭", command=sonicwall_ssl_vpn_gui, width=45,
                     bootstyle="warning")
...
button12 = ttk.Button(group3, text="DVR 登录绕过漏洞(CVE-2018-9995)一把梭", command=dvr_login_bypass_gui, width=45,
                      bootstyle="info")
button12.grid(row=2, column=4, columnspan=2, padx=5, pady=5)
button13 = ttk.Button(group3, text="MetaBase任意文件读取漏洞(CVE-2021-41277)一把梭", command=metabase_readfile_gui, width=45,
                      bootstyle="primary")
